...
MSE0 = np.mean( abs( im - inp )**2 ) #Calculation of the Mean-Absolute-Error
...
plt.xlabel('MSE = ' + str(np.float16(MSE0)))  

plt.subplot(133)
plt.imshow(abs(out), cmap='gray')
plt.title('Processed Image (Output)')
plt.xlabel('MSE = ' + str(np.float16(MSE[q])))  
